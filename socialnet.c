...
struct node*refineUser(struct node*user, struct node *users)
{
    ...
    if(users==NULL){
        ...
        while(temp!=NULL){
            if(user->id==temp->id) user->id++;
            else if(user->id<temp->id) temp = temp->left;
        ...
        for(i=0;i<user->numfren;i++){
            ...
            else if(user->friends[i]<temp->id) temp = temp->left;
            else temp = temp->right;
            }
            if(temp==NULL){
                for(k=i+1;k<user->numfren;k++) user->friends[k-1] = user->friends[k];
                user->numfren--;
                i--;
            }
        }
...
struct node* insertUser(struct node*root,int id,struct node*user)
{
   ...
   if(root==NULL) return user;
    ...
    if(id<root->id) root->left = insertUser(root->left,user->id,user);
    else root->right = insertUser(root->right,user->id,user);
   }
   return root;
}
    ...
    if(users==NULL) return;
        ...
        for(i=0;i<users->numfren;i++){
            if(key==users->friends[i]){
                for(j=i+1;j<users->numfren;j++) users->friends[j-1] = users->friends[j];
                users->numfren--;
            }
        }
    }
